...
`;
chatWrap.innerHTML = `
  <div id="chatLog" style="max-height: 180px; overflow:auto; padding: 12px; font-size: 12.5px; line-height:1.35;"></div>
`;
...
clearOverlay.innerHTML = `
  ...
  ">
    ...
    <div style="margin-top: 8px; font-size: 15px; opacity: 0.92;">
      <b>눈사람 모자 퀘스트</b> + <b>15명 질문 답변</b> 완료!<br/>
      아래 <b>인증코드</b>가 보이게 스크린샷을 찍어서 <b>공지방 이벤트 글에 댓글</b>로 달아주세요. 🔥
    </div>
...
quizModal.innerHTML = `
  ...
  ">
    <div id="quizNpcName" style="font-size: 20px; font-weight: 900; margin-bottom: 8px; color: #ffdd66;"></div>
    <div id="quizQuestion" style="font-size: 16px; font-weight: 600; margin-bottom: 20px; line-height: 1.5;"></div>
    ...
    "></div>
    <div id="quizChoices" style="display: flex; flex-direction: column; gap: 10px;"></div>
...
function closeQuizModal() {
    const modalContent = quizModal.querySelector("#quizModalContent");
...
quizModal.querySelector("#closeQuizModal").addEventListener("click", closeQuizModal);

// 선택지 버튼 클릭 핸들러
function showQuizModal(resident) {
    // 이미 모달이 열려 있으면 무시 (문제 푸는 중 재접촉 방지)
    ...
    if (!st || st.solved) {
        setMsg(`${resident.name}: 이미 문제를 푼 사람입니다!`, 3500);
        return;
    }

    const questionData = st.q;
    quizModal.querySelector("#quizNpcName").textContent = `${resident.name}님의 질문`;
    quizModal.querySelector("#quizQuestion").textContent = questionData.q;

    // 피드백 초기화
    const feedbackDiv = quizModal.querySelector("#quizFeedback");
    feedbackDiv.style.display = "none";

    const choicesDiv = quizModal.querySelector("#quizChoices");
    ...
    questionData.choices.forEach(choice => {
        ...
        btn.addEventListener("click", () => {
            checkAnswer(resident, choice, questionData.answer);
        });
    ...
    setTimeout(() => {
        ...
        quizModal.querySelector("#quizModalContent").style.transform = "scale(1)";
    }, 10);
}

function checkAnswer(resident, selectedAnswer, correctAnswer) {
    const feedbackDiv = quizModal.querySelector("#quizFeedback");
    ...
    if (selectedAnswer === correctAnswer) {
        ...
        feedbackDiv.style.border = "2px solid rgba(34, 197, 94, 0.5)";
        ...
        chatPush("SYSTEM", "정답! ✅");
        setMsg(`${resident.name}: 정답입니다! ✅`, 3500);
        solveNpcQuizCorrect(resident);
    } else {
        // 오답 피드백
        feedbackDiv.textContent = "❌ 오답! 다시 시도해보세요!";
        feedbackDiv.style.display = "block";
        feedbackDiv.style.background = "rgba(239, 68, 68, 0.2)";
        feedbackDiv.style.border = "2px solid rgba(239, 68, 68, 0.5)";
...
function chatPushWithLink(who, text, link) {
    const line = document.createElement("div");
    line.style.marginBottom = "6px";
    line.innerHTML = `<span style="opacity:.85; font-weight:900;">${who}:</span> <a href="${link}" target="_blank" style="opacity:.95; color: #F97316">${text}</a>`;
    chatLog.appendChild(line);
    chatLog.scrollTop = chatLog.scrollHeight;
}
...
// -----------------------------------------------------
const NPC_QUESTIONS = {
    "최원빈": { q: "내가 좋아하는 야구 프로팀은?", choices: ["두산", "한화", "LG", "롯데"], answer: "한화" },
    "김지원": { q: "내가 취미로 하는 운동은?", choices: ["농구", "달리기", "야구", "수영"], answer: "수영" },
    "조하정": { q: "내가 즐겨하는 취미는?", choices: ["등산", "영화감상", "산책", "요리"], answer: "산책" },
    "김은채": { q: "내가 좋아하는 야구 프로팀은?", choices: ["삼성", "한화", "LG", "기아"], answer: "삼성" },
    "조용석": { q: "나의 MBTI는?", choices: ["INTP", "ENFP", "INTJ", "ENTJ"], answer: "INTP" },
    "안형욱": { q: "내 주인님(고양이) 이름은?", choices: ["고등어", "치즈", "꿀잼", "망치"], answer: "꿀잼" },

    "소수경": { q: "나의 MBTI는?", choices: ["ENFP", "ISTP", "INTJ", "ENTJ"], answer: "ISTP" },
    "신현민": { q: "내 군생활 보직은?", choices: ["취사병", "포병", "헌병", "보병"], answer: "취사병" },
    "고은지": { q: "나의 MBTI는?", choices: ["ENFP", "ISFJ", "INTJ", "ENTJ"], answer: "ISFJ" },
    "정태규": { q: "내가 즐겨하는 취미는?", choices: ["등산", "게임", "야구", "영화감상"], answer: "게임" },
    "김소현": { q: "내가 즐겨하는 취미는?", choices: ["영화감상", "야구", "요가", "등산"], answer: "요가" },
    "정지훈": { q: "내가 구사할 수 있는 언어는?", choices: ["독일어", "필란드어", "중국어", "일본어"], answer: "일본어" },

    "정민교": { q: "내가 즐겨하는 취미는?", choices: ["방탈출", "여행", "카페탐방", "등산"], answer: "방탈출" },
    "최길중": { q: "나의 MBTI는?", choices: ["INTP", "ENFP", "INTJ", "ENTJ"], answer: "INTJ" },
    "류호정": { q: "내가 즐겨하는 취미는?", choices: ["등산", "EDM", "클래식", "뉴에이지"], answer: "EDM" },
    "박소영": { q: "나의 MBTI는?", choices: ["INTP", "ESTP", "INTJ", "ENTJ"], answer: "ESTP" },
    "박수지": { q: "나의 MBTI는?", choices: ["INTP", "ESTP", "INTJ", "ENFJ"], answer: "ENFJ" },
    "배강혁": { q: "내가 전역했을 때의 계급은?", choices: ["중위", "병장", "중사", "대위"], answer: "대위" },

    "김인목": { q: "나의 MBTI는?", choices: ["ISTP", "ENFP", "INTJ", "ENTJ"], answer: "ISTP" },
    "김예은": { q: "나의 MBTI는?", choices: ["ISTP", "ENFP", "INTJ", "ENTJ"], answer: "INTJ" },
    "이지민": { q: "내가 즐겨하는 취미는?", choices: ["농구", "게임", "등산", "테니스"], answer: "게임" },
    "정호진": { q: "내가 즐겨하는 취미는?", choices: ["독서", "영화", "요가", "코딩"], answer: "영화" },
    "임호진": { q: "나의 MBTI는?", choices: ["ISTP", "ENFP", "INTJ", "ENTJ"], answer: "ENTJ" },
    "박경화": { q: "나의 MBTI는?", choices: ["INTP", "ENFP", "INTJ", "ENTJ"], answer: "INTP" },

    "황도윤": { q: "내가 즐겨하는 취미는?", choices: ["애니", "당구", "야구", "등산"], answer: "애니" },
    "이중현": { q: "나의 MBTI는?", choices: ["INTP", "ENFP", "INFJ", "ENTJ"], answer: "INFJ" },
    "문혜린": { q: "나의 피크민 블룸 닉네임은?", choices: ["무너", "햇살", "문어", "지니"], answer: "무너" },
    "강태훈": { q: "나의 MBTI는?", choices: ["ISTP", "ENFP", "INFJ", "ENTJ"], answer: "ISTP" },
    "박주빈": { q: "나의 MBTI는?", choices: ["ISFP", "ENFP", "INFJ", "ENTJ"], answer: "ISFP" },
    "성기찬": { q: "나의 MBTI는?", choices: ["ISFP", "ENFP", "INFJ", "ENTJ"], answer: "ENTJ" },

    "권지원": { q: "내가 좋아하는 동물은?", choices: ["강아지", "고양이", "여우", "햄스터"], answer: "고양이" },
    "박영수": { q: "내가 주로 플레이하는 롤 캐릭터는?", choices: ["진", "알리스타", "애니비아", "신짜오"], answer: "애니비아" },
    "이승현": { q: "내가 즐겨하는 운동은?", choices: ["풋살", "야구", "클라이밍", "농구"], answer: "풋살" },
    "정채림": { q: "내가 즐겨하는 취미는?", choices: ["음악감상", "여행", "게임", "영화감상"], answer: "게임" },
    "이재민": { q: "내가 즐겨하는 취미는?", choices: ["볼링", "여행", "주식투자", "등산"], answer: "주식투자" },
    "이지혜": { q: "나의 MBTI는?", choices: ["ISFP", "ENFP", "INFJ", "ENTJ"], answer: "ENTJ" },
...
const skyMat = new THREE.ShaderMaterial({
    ...
    uniforms: {
        uTime: { value: 0 },
        uTop: { value: new THREE.Color(0x1a0a28) },
        uBottom: { value: new THREE.Color(0x0a0510) },
    },
    vertexShader: `
    varying vec3 vPos;
    void main(){
  ...
  `,
    ...
    void main(){
      ...
      vec3 aurA = vec3(1.00, 0.85, 0.20);
      vec3 aurB = vec3(1.00, 0.40, 0.20);
      vec3 aurC = vec3(0.80, 0.20, 1.00);
...
function addRealisticCone(radius, height, y, color, segments = 64) {
    // 메인 원뿔 (불규칙한 모양)
    const geo = new THREE.ConeGeometry(radius, height, segments, 3, false);
    const positions = geo.attributes.position;

    // 가지의 불규칙성 추가 (자연스러운 모양)
    for (let i = 0; i < positions.count; i++) {
        const x = positions.getX(i);
        const y = positions.getY(i);
        const z = positions.getZ(i);

        if (y < height * 0.45) { // 아랫부분만 불규칙하게
            const dist = Math.sqrt(x * x + z * z);
            const noise = (Math.random() - 0.5) * 0.12 * dist;
    ...
    // 작은 가지들 추가 (입체감)
    const branchCount = Math.floor(radius * 15);
    for (let i = 0; i < branchCount; i++) {
        const angle = (i / branchCount) * Math.PI * 2 + Math.random() * 0.3;
        const branchY = Math.random() * height * 0.7;
        const branchRadius = radius * (1 - branchY / height) * 0.9;

        const branchGeo = new THREE.ConeGeometry(0.08, 0.25 + Math.random() * 0.15, 6);
        const branch = new THREE.Mesh(branchGeo, mat);

        const x = Math.cos(angle) * branchRadius;
        const z = Math.sin(angle) * branchRadius;
        branch.position.set(x, y + branchY, z);

        // 바깥쪽을 향하도록 회전
        branch.rotation.z = Math.PI * 0.35 + Math.random() * 0.15;
    ...
    // 눈 쌓임 (불규칙하게)
    const snowGeo = new THREE.ConeGeometry(radius * 1.01, height * 0.16, segments, 1, true);
    ...
    const cap = new THREE.Mesh(
        snowGeo,
        new THREE.MeshStandardMaterial({ color: 0xf5f9ff, roughness: 0.95 })
    );
    cap.position.y = y + height * 0.35;
    cap.receiveShadow = true;
    cap.rotation.y = Math.random() * Math.PI;
    tree.add(cap);
}

addRealisticCone(2.50, 3.80, 1.35, 0x0a4d22);  // 가장 아래 - 가장 어두운 녹색
addRealisticCone(2.15, 3.30, 1.90, 0x0c5627);  // 아래
addRealisticCone(1.75, 2.85, 2.50, 0x0e6a2e);  // 중간
addRealisticCone(1.35, 2.35, 3.05, 0x107832);  // 위
addRealisticCone(0.90, 1.80, 3.55, 0x128a38);  // 가장 위 - 가장 밝은 녹색

const trunkGeo = new THREE.CylinderGeometry(0.30, 0.40, 1.50, 32, 4);
...
for (let i = 0; i < trunkPositions.count; i++) {
    ...
    trunkPositions.setXYZ(i, x + noise, y, z + noise);
}
...
star.position.set(0, 5.15, 0);
star.castShadow = true;
tree.add(star);

const starGlow = new THREE.PointLight(0xffd76a, 1.1, 16, 2);
starGlow.position.set(0, 5.25, 0.25);
tree.add(starGlow);
...
starSprite.position.set(0, 5.25, 0.2);
tree.add(starSprite);
...
(function addGarland() {
    ...
    for (let i = 0; i <= 280; i++) {
        const tt = i / 280;
        const y = 1.40 + tt * 3.65;
        const r = 0.30 + (1 - tt) * 2.45;
...
(function initPlayer() {
    ...
    setMsg("🎯 미션: 모자 퀘스트 + 인물 퀴즈(15명에게서 1문제씩)!", 6200);
    chatPush("SYSTEM", "미션: (1) 모자 줍고 눈사람에게 전달 (2) 랜덤 주민 15명에게서 1문제씩 풀기");
})();
...
function updateHUD() {
    ...
    const totalNPCs = mission.quizNPCs.length;
    const quizLine = `🙆 퀴즈: ${mission.quizSolved ? "완료" : `진행: ${mission.quizSolvedCount}/${totalNPCs}`}`;

    const hintLine = mission.allClear
        ? `✅ ALL CLEAR! 인증코드 보이게 스샷 → 공지방 이벤트 글 댓글!`
        : `힌트: 대상 15명에게 부딪치기 / 모자: 오른쪽, 눈사람: 왼쪽`;

    hud.innerHTML = `
    <div style="font-size: 14px; font-weight: 900; margin-bottom: 6px;">🎄 NewYear Missions</div>
    <div>${hatLine}</div>
    <div style="margin-top: 2px;">${quizLine}</div>
    <div style="margin-top: 6px; font-size: 12px; opacity: 0.95; font-weight: 800;">🙋‍♀️ 대상 15명: ${list || "(선정 중)"}</div>
...
(function initQuizMission() {
    mission.quizNPCs = pickUnique(residents, 15); // 랜덤 15명
    ...
    // 선정된 NPC에게만 개인 질문 할당
    for (const npc of mission.quizNPCs) {
        const question = NPC_QUESTIONS[npc.name];
        if (question) {
            mission.quizByNpc.set(npc.name, { q: question, solved: false });
        }
    }
    ...
    chatPush("SYSTEM", `퀴즈 대상 15명: ${names}`);
    chatPush("SYSTEM", "내 캐릭터(me)와 퀴즈 대상 주민이 부딪히면 질문과 선택지가 나타납니다. 정답을 선택하세요!");
})();
...
function showNpcQuizQuestion(resident) {
    ...
    const st = quizStateFor(resident);
    if (!st || st.solved) {
        setMsg(`${resident.name}: 이미 문제를 푼 사람입니다!`, 4200);
        return;
    }

    // 선택지 모달 표시
    showQuizModal(resident);
}
...
function solveNpcQuizCorrect(resident) {
    ...
    if (mission.quizSolvedCount >= totalNPCs) {
        mission.quizSolved = true;
        updateHUD();
        setMsg(`✅ 모든 질문 완료! (${totalNPCs}/${totalNPCs})`, 5200);
        chatPush("SYSTEM", `모든 주민의 질문을 맞췄습니다! 이제 모자 퀘스트(미완료 시)만 남았다.`);
        tryCompleteAllClear();
        return;
    }

    updateHUD();
    setMsg(`✅ 정답! (${mission.quizSolvedCount}/${totalNPCs}) 다음 주민에게 가보세요!`, 4200);
    chatPush("SYSTEM", `정답! (${mission.quizSolvedCount}/${totalNPCs})`);
}
...
function checkHatQuest() {
    ...
    if (mission.playerHasHat && !mission.hatDelivered && snowmanTarget) {
        if (distXZ(snowmanTarget.position, player.obj.position) <= 1.10) {
            ...
            chatPush("SYSTEM", "모자 퀘스트 완료! 이제 15명의 질문에 답하기만 남았다.");
            updateHUD();
            tryCompleteAllClear();
        }
    }
...
// -----------------------------------------------------
const PLAYLIST = [
    "Where the Sun Once Rested.mp3",
    "Start From Here.mp3",
    "Only Breathing in the Dark Room.mp3",
    "Yesterday’s Me in the Window.mp3",
];
...
setTimeout(() => {
    ...
    chatPush("SYSTEM", "질문에 대한 답안은 멤버카드에서 확인할 수 있습니다. 멤버카드를 꼭 확인하면서 풀어주세요.");
    chatPushWithLink("SYSTEM", "멤버카드 클릭", "https://www.notion.so/teamsparta/2c42dc3ef514807c9b43d382b04599e9?v=2c42dc3ef51481609c1f000cdbe20d5d")
